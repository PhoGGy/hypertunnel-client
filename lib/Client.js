...
class Client {
  ...
  async beacon() {
	debug('Beacon')
	const { body } = await got(`${this.server}/beacon/${this.internetPort}`, { json: true,throwHttpErrors: false })
	if (!body.success) { throw new Error(body && body.message ? body.message : 'Wrong response to beacon, maybe server got restarted?') }
  }

  beaconInterval () {
    setInterval(() => {
      this.beacon()
    }, 15 * 1000) // Run every 15 sec
  }
